...
export default function ProfileTab({
  ...
  async function handleImageFileChange(file: File | null) {
    ...
    try {
      ...
      const uploadedUrl = await uploadToWasabi(file)
      setProductImage(uploadedUrl)
      setSavedImageDataUrl(uploadedUrl)
    } catch (error) {
      console.error(error)
      alert('Product image upload mislukt')
    } finally {
  ...
  async function handleVideoFileChange(file: File | null) {
    ...
    try {
      ...
      console.error(error)
      alert('Product video upload mislukt')
    } finally {
  ...
  async function handleBrandingLogoFileChange(file: File | null) {
    ...
    try {
      ...
      console.error(error)
      alert('Shop logo upload mislukt')
    } finally {
  ...
  async function handleBrandingAvatarFileChange(file: File | null) {
    ...
    try {
      ...
      console.error(error)
      alert('Admin avatar upload mislukt')
    } finally {
      ...
      <div style={{ paddingBottom: 110, maxWidth: 520, margin: '0 auto', color: theme.textColor }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={panelStyle(theme)}>
            ...
            <label style={uploadLabelStyle(theme)}>
              {isUploadingProductImage ? 'Uploading product image...' : 'Upload Foto'}
              <input
                type="file"
                accept="image/*"
            ...
            <label style={uploadLabelStyle(theme)}>
              {isUploadingProductVideo ? 'Uploading product video...' : 'Upload Video'}
              <input
                type="file"
                accept="video/*"
